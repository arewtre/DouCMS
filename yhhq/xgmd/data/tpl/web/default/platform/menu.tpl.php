...
<ul class="nav nav-tabs">
	<li class="<?php  if($_GPC['status'] == 'normal' || $_GPC['status'] == '') { ?>active<?php  } ?>"><a href="<?php  echo url('platform/menu/display', array('status' => 'normal'));?>">自定义菜单</a></li>
	<li class="<?php  if($_GPC['status'] == 'history') { ?>active<?php  } ?>"><a href="<?php  echo url('platform/menu/display', array('status' => 'history'));?>">历史菜单</a></li>
</ul>
<?php  if($do == 'display') { ?>
			...
			<table class="table table-hover">
				...
				<tr>
					...
					<td align="right">
						...
						<?php  } ?>
						<?php  if($da['type'] != 1) { ?>
						<a href="<?php  echo url('platform/menu/remove', array('id' => $da['id'], 'status' => $_GPC['status']));?>" onclick="if(!confirm('<?php  if($da['type'] == 1) { ?>删除默认菜单会清空所有菜单记录，确定吗？<?php  } else { ?>确定删除菜单?<?php  } ?>')) return false;" class="btn btn-default">删除</a>
						<?php  } ?>
					</td>
				</tr>
				<?php  } } ?>
