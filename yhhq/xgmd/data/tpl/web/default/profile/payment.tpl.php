...
<script type="text/javascript">
	...
	require(['angular'], function(angular){
		...
		//设置余额支付开关下拉框选择
		$('.select-back li a').click(function(){
			var back = $(this).text();
			$(this).parent().parent().prev().prev().find('.button-value').text(back);
			if(back == '元') {
				$(this).parent().parent().prev().attr('value','credit2');
			}else{
				$(this).parent().parent().prev().attr('value','credit1');
			}
		})
		$(document).on('click','.select-back li a',function(){
			var back = $(this).text();
			$(this).parent().parent().prev().prev().find('.button-value').text(back);
			if(back == '元') {
				$(this).parent().parent().prev().attr('value','credit2');
		...
		$('#recharge-add-credit2').click(function(){
			var html = '<div class="form-group item">' +
							'<label class="col-xs-12 col-sm-3 col-md-2 control-label"></label>' +
							'<div class="col-sm-6">' +
								'<div class="input-group">' +
									'<span class="input-group-addon">充</span>' +
									'<input type="text" name="recharge[items][]" value="" class="form-control">' +
									'<span class="input-group-addon">元</span>' +
									'<span class="input-group-addon">返</span>' +
									'<input type="text" name="back[items][]" value="" class="form-control">' +
									'<div class="input-group-btn">' +
									'<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="width:66px;"><span class="button-value">元</span> <span class="caret"></span></button>' +
									'<input type="hidden" name="rechargeType[items][]" value="credit2"/>' +
						...
						'</div>' + 
							...
							'</div>' +
						'</div>';
			if($('#recharge .recharge-credit2-show .item').size() >= 4) {
				util.message('最多可添加4个充值规则', '', 'error');
				return false;
			}
			$('#recharge .recharge-credit2-show').append(html);
		});
		$(document).on('click','.recharge-del',function(){
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-sm-9 col-xs-12">
						...
						<div class="alert alert-warning">
                            ...
                            <p>支付授权目录: <?php  echo $_W['siteroot'];?>/app/</p>
				...
				<div ng-show="wechat.switch == 'true'">
					...
					<div class="form-group s" id="apikey" <?php  if($pay['wechat']['version'] == 1) { ?> style="display:none;" <?php  } ?>>
						...
						<div class="col-sm-9 col-xs-12">
							...
							<span class="help-block">此值需要手动在腾讯商户后台API密钥保持一致。<?php  if($_W['isfounder']) { ?><a href="http://lejiplaza.com/thread-5788-1-1.html" target="_blank">查看设置教程</a><?php  } ?></span>
						</div>
					</div>
				</div>
				<?php  } else { ?>
				<div class="form-group">
					...
					<div class="col-sm-9 col-xs-12">
						<div class="alert alert-warning">
							你必须向微信公众平台提交企业信息以及银行账户资料，审核通过并签约后才能使用微信支付功能 <a href="http://lejiplaza.com/thread-1354-1-1.html" target="_blank">申请及详情请查看这里</a>.
						</div>
