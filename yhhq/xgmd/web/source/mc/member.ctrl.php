<?php
require '../framework/function/crm.func.php';
...
if($do == 'display') {
	...
	$sql = "SELECT uid, uniacid, groupid, realname, nickname, mobile,avatar, createtime  FROM ".tablename('mc_members')." WHERE uniacid = '{$_W['uniacid']}' ".$condition." ORDER BY createtime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
	$list = pdo_fetchall($sql);
// 	if(!empty($list)) {
// 		foreach($list as &$li) {
// 			if(!empty($li['mobile'])) {
// 			    $access_token = json_decode(get_access_token())->access_token;
// 			    $params = array(
// 			        'p_cond'=>$li['mobile'],
// 			        'p_mode'=>2,
// 			        'p_mkt'=>'201'
// 			    );
// 			    $result = get_crm_response($access_token, $params, 8002);
// 			    $crm_user_array = json_decode($result)->data;
// 			}
// 		}
// 	}
...
if($do == 'post') {
	...
	if(!empty($profile)) {
		if(empty($profile['email']) || (!empty($profile['email']) && substr($profile['email'], -6) == 'lejiplaza.com' && strlen($profile['email']) == 39)) {
...
// 	}
	...
	$result = get_crm_response($access_token, $params, 8002);
	$crm_user_array = json_decode($result)->data;
	if($crm_user_array != null){
	    $crm_user = $crm_user_array['0'];
	    $cdmno = $crm_user->cdmno;
	    $ctname = $crm_user->ctname;
	    $chisjfye = $crm_user->ccurjfye;
	    $chisxfje = $crm_user->ccurxfje;
	}
}
template('mc/member');