...
if ($do == "post") {
	if ($_GPC ['add'] == 1) {
		...
		pdo_insert ( 'coupon', $coupon );
 		...
 		$arr['coupon_info_pic'] = $_GPC ['photo_list'];
 		$arr['coupon_type'] = 1;
 		$arr['sort'] = $_GPC ['sort'];
...
if ($do == "display") {
	...
	$totalnum = pdo_fetchcolumn( "SELECT COUNT(*) FROM ims_coupon where dis_type=1");
...
if ($do == "view") {
    ...
    //pre($_GPC);
    if ($_GPC ['add'] == 1) {       
        pdo_update( 'coupon', array ('name' =>$_GPC['coupon_name'],'coupon_desc' =>$_GPC['coupon_desc']), array ('coupon_id' => $id ) );
        pdo_update( 'coupon_info', array ('coupon_info_pic' =>json_encode($_GPC['photo_list'])), array ('id' => $id ) );
        message ( '优惠券更新成功！', url ('coupon/list', array ('do' => 'display') ), 'success' );
    }    
}
...
if($do=="send"){
    ...
    $re = pdo_fetch( "SELECT * FROM ims_coupon  WHERE coupon_id='{$couponid}'");
    if(!empty($res)){
    ...
    if($re['use_time_type']==1){
        $time = date ( "Y-m-d H:i:s" );
        if($re['use_time_end']< $time){
            die(json_encode(array("result"=>0,"msg"=>"此优惠券已过有效期!,无法发放!")));
        }
    }
    $num = $_GPC['num'];
    $code=array();
