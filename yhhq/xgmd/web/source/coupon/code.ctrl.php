...
function code_all($pindex = 1, $psize = 10,$filter = array()) {
	...
	$total = pdo_apk_fetchcolumn('SELECT COUNT(*) FROM cp_coupon_code' . $condition, $params);
	$coupons = pdo_apk_fetchall('SELECT code.*, coupon.name coupon_name FROM cp_coupon_code code left join cp_coupon coupon on code.coupon_id=coupon.coupon_id '. $condition . ' ORDER BY add_time DESC ' . $limit, $params,'id');
