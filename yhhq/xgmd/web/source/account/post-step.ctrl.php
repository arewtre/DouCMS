...
if ($step == 1) {
    ...
    if (checksubmit('getinfo')) {
        ...
        if (! empty($username) && ! empty($password)) {
            ...
            if (is_error($loginstatus)) {
                message('模拟登陆微信公众平台出错,错误详情:' . $loginstatus['message'], url('account/post-step', array(
                    'uniacid' => $uniacid,
                    'step' => 1
                )), 'error');
            }
            ...
            if (empty($basicinfo['name'])) {
                message('一键获取信息失败,请手动设置公众号信息！错误详情:' . $basicinfo['message'], url('account/post-step/', array(
                    'uniacid' => $uniacid,
                    'step' => 2
                )), 'error');
            }
            ...
            message('请填写公众平台用户名和密码', url('account/post-step', array(
                'uniacid' => $uniacid,
                'step' => 1
            )), 'error');
        }
    ...
    if (checksubmit('submit')) {
        ...
        if (empty($uniacid)) {
            ...
            $unisettings['creditbehaviors'] = array(
                'activity' => 'credit1',
                'currency' => 'credit2'
            );
            ...
            $unisettings['sync'] = iserializer(array(
                'switch' => 0,
                'acid' => ''
            ));
            pdo_insert('uni_settings', $unisettings);
            
            pdo_insert('mc_groups', array(
                'uniacid' => $uniacid,
                'title' => '默认会员组',
                'isdefault' => 1
            ));
        ...
        if (empty($acid)) {
            ...
            if (is_error($acid)) {
                message('添加公众号信息失败', '', url('account/post-step/', array(
                    'uniacid' => intval($_GPC['uniacid']),
                    'step' => 2
                ), 'error'));
            }
            pdo_update('uni_account', array(
                'default_acid' => $acid
            ), array(
                'uniacid' => $uniacid
            ));
            ...
            unset($update['type']);
            pdo_update('account_wechats', $update, array(
                'acid' => $acid,
                'uniacid' => $uniacid
            ));
        }
        ...
        if (! empty($_FILES['qrcode']['tmp_name'])) {
            ...
            $_W['uploadsetting']['image']['extentions'] = array(
                'jpg'
            );
        ...
        if (! empty($_GPC['uniacid']) || empty($_W['isfounder'])) {
            header("Location: " . url('account/post-step/', array(
                'uniacid' => $uniacid,
                'acid' => $acid,
                'step' => 4
            )));
    ...
    if ($do == 'edit_sms') {
        ...
        $settings = uni_setting($uniacid, array(
            'notify'
        ));
        ...
        pdo_update('uni_settings', $updatedata, array(
            'uniacid' => $uniacid
        ));
        message(error(1, array(
            'count' => $count_num,
            'num' => $num
        )), '', 'ajax');
    }
    if ($do == 'userinfo') {
        ...
        $result['uid'] = $user['uid'];
        $result['group'] = pdo_fetch("SELECT id, name, package FROM " . tablename('users_group') . " WHERE id = :id", array(
            ':id' => $user['groupid']
        ));
    ...
    if (checksubmit('submit')) {
        ...
        if (! empty($_GPC['signature'])) {
            ...
            $setting = pdo_get('uni_settings', array(
                'uniacid' => $_W['uniacid']
            ));
        ...
        if (! empty($uid)) {
            pdo_delete('uni_account_users', array(
                'uniacid' => $uniacid,
                'uid' => $uid
            ));
            $owner = pdo_get('uni_account_users', array(
                'uniacid' => $uniacid,
                'role' => 'owner'
            ));
            if (! empty($owner)) {
                pdo_update('uni_account_users', array(
                    'uid' => $uid
                ), array(
                    'uniacid' => $uniacid,
                    'role' => 'owner'
        ...
        pdo_delete('uni_account_group', array(
            'uniacid' => $uniacid
        ));
        ...
        if (! empty($_GPC['extra']['modules']) || ! empty($_GPC['extra']['templates'])) {
            ...
            $id = pdo_fetchcolumn("SELECT id FROM " . tablename('uni_group') . " WHERE uniacid = :uniacid", array(
                ':uniacid' => $uniacid
            ));
            if (empty($id)) {
        ...
        module_build_privileges();
        if (! empty($_GPC['from'])) {
            message('公众号权限修改成功', url('account/post-step/', array(
                'uniacid' => $uniacid,
                'step' => 3,
                'from' => 'list'
            )), 'success');
    ...
    $notify = $settings['notify'] ? $settings['notify'] : array();
    $ownerid = pdo_fetchcolumn("SELECT uid FROM " . tablename('uni_account_users') . " WHERE uniacid = :uniacid AND role = 'owner'", array(
        ':uniacid' => $uniacid
    ));
    if (! empty($ownerid)) {
        ...
        $owner['group'] = pdo_fetch("SELECT id, name, package FROM " . tablename('users_group') . " WHERE id = :id", array(
            ':id' => $owner['groupid']
        ));
        $owner['group']['package'] = iunserializer($owner['group']['package']);
    }
    $extend = pdo_fetch("SELECT * FROM " . tablename('uni_group') . " WHERE uniacid = :uniacid", array(
        ':uniacid' => $uniacid
    ));
    $extend['modules'] = iunserializer($extend['modules']);
    $extend['templates'] = iunserializer($extend['templates']);
    if (! empty($extend['modules'])) {
        $owner['extend']['modules'] = pdo_getall('modules', array(
            'name' => $extend['modules']
        ));
    }
    if (! empty($extend['templates'])) {
        $owner['extend']['templates'] = pdo_getall('site_templates', array(
            'id' => $extend['templates']
        ));
    }
    $extend['package'] = pdo_getall('uni_account_group', array(
        'uniacid' => $uniacid
    ), array(), 'groupid');
