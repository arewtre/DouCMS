...
// 		  }
		    message ( '优惠券添加成功！', url ('coupon/park_list', array ('do' => 'display') ), 'success' );
...
if ($do == "display") {
	...
	$condition = ' WHERE dis_type =1 AND type=4';
	...
	$coupons = pdo_fetchall( 'SELECT * FROM ims_coupon ' . $condition . ' ORDER BY reg_time DESC ' . $limit, $params, 'coupon_id' );
...
if ($do == "view") {
    $id = intval ( $_GPC ['id'] );
    $item = pdo_fetch ( "SELECT * FROM ims_coupon WHERE coupon_id = :coupon_id", array (':coupon_id' => $id)); 
...
if ($do == "check") {
	...
	if ($_GPC ['m'] == "pass") {
		...
		message ( '共'. $num .'条停车券审核通过！', url ('coupon/park_list', array ('do' => 'display') ), 'success' );
	}
	if ($_GPC ['m'] == "unPass") {
	   foreach($id as $v){
		      //pdo_update( 'ims_coupon', array ('check_state' =>-1,'reg_time'=>$time), array ('coupon_id' => $v ) );
	    ...
	    message ( '共'. $num .'条停车券被拒绝！', url ( 'coupon/park_list', array ('do' => 'display') ), 'success' );
	}
