...
$psize = 15;
if ($do == "display") {
	$condition = ' WHERE dis_type=1 AND check_state = 0 AND issue_time_end >now() AND type=4';
	$params = array ();
	$limit = ' LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
	$total = pdo_fetchcolumn ( "SELECT COUNT(*) FROM ims_coupon " . $condition, $params );
	$coupons = pdo_fetchall ( 'SELECT * FROM ims_coupon ' . $condition . ' ORDER BY reg_time DESC ' . $limit, $params, 'id' );
	$coupons = array (
			'total' => $total,
			'coupons' => $coupons 
	);
