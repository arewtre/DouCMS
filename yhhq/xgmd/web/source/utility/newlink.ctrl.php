...
elseif ($do == 'articlelist') {
	...
	$sql = "SELECT * FROM ". tablename('store') . " WHERE 1 = 1 $condition LIMIT ". ($pindex - 1) * $psize . ',' . $psize;
	$result['list'] = pdo_fetchall($sql, array(), 'id');
	$total  = pdo_fetchcolumn("SELECT count(*) FROM ims_store WHERE 1=1 $condition ");
	$result['pager'] = pagination($total, $pindex, $psize, '', array('before' => '2', 'after' => '3', 'ajaxcallback'=>'null'));
}elseif ($do == 'foodlink') {
    //echo"<pre>";print_r($_GPC);die;
    $result = array();
	...
	if (!empty($_GPC['keyword'])) {
		$condition .= " AND promotion_name LIKE '%{$_GPC['keyword']}%'  ";
	} 	
	$sql = "SELECT * FROM ". tablename('promotion') . " WHERE 1 = 1 $condition LIMIT ". ($pindex - 1) * $psize . ',' . $psize;
	$result['list'] = pdo_fetchall($sql, array(), 'id');
	$total  = pdo_fetchcolumn("SELECT count(*) FROM ims_promotion WHERE 1=1 $condition ");
	$result['pager'] = pagination($total, $pindex, $psize, '', array('before' => '2', 'after' => '3', 'ajaxcallback'=>'null'));
