...
if($_W['ispost'] && $_W['isajax']) {
    ...
    if($_W["user"]["credit2"]<=0) die(json_encode(array("code"=>1, "message"=>"余额不足,请进行充值",url("member/recharge"))));
    ...
    $total = $price*$number;
    if($_W["user"]["credit2"]< $total) die(json_encode(array("code"=>1, "message"=>"余额不够,请进行充值",url("member/recharge"))));
...
if($_W["user"]["credit2"] < doubleval($settings["dx_UnitPrice"])) {
    message("余额不足,请进行充值.",url('member/recharge'));
}
