(function(window) {
	...
	}; // end of emoji dialog  weblinkBrowser
...
//////////////////自定义的///////////
	util.weblinkBrowser = function(callback){
		var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>';
		var modalobj = util.dialog('请填写URL地址',['./index.php?c=utility&a=newlink&do=weblink&callback=webLinkComplete'],footer,{containerName:'link-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-body').css({'height':'300px','overflow-y':'auto' });
		modalobj.modal('show');

		window.webLinkComplete = function(link){
			if($.isFunction(callback)){
				callback(link);
				modalobj.modal('hide');
			}
	...
	util.activeBrowser = function(callback, page){
		var footer = '';
		var modalobj = util.dialog('请选择活动',['./index.php?c=utility&a=newlink&do=activelink&callback=activeLinkComplete&page='+ page],footer,{containerName:'link-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-body').css({'height':'700px','overflow-y':'auto' });
		modalobj.modal('show');

		window.activeLinkComplete = function(link, page){
			if($.isFunction(callback)){
				callback(link, page);
				if (page == '' || page == undefined) {
					modalobj.modal('hide');
	...
	util.activelinkBrowser = function(callback, page, key){
		var footer = '';
		var modalobj = util.dialog('请选择活动',['./index.php?c=utility&a=newlink&do=activelink&callback=activeLinkComplete&page='+ page+'&keyword='+key],footer,{containerName:'link-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-body').css({'height':'700px','overflow-y':'auto' });
		modalobj.modal('show');

		window.activeLinkComplete = function(link, page ,key){
			if($.isFunction(callback)){
				callback(link, page ,key);
				if (page == '' || page == undefined) {
					modalobj.modal('hide');
				}
			}
	...
	util.storeBrowser = function(callback, page){
		var footer = '';
		var modalobj = util.dialog('请选择商户',['./index.php?c=utility&a=newlink&do=storelink&callback=storeLinkComplete&page='+ page],footer,{containerName:'link-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-body').css({'height':'700px','overflow-y':'auto' });
		modalobj.modal('show');

		window.storeLinkComplete = function(link, page){
			if($.isFunction(callback)){
				callback(link, page);
				if (page == '' || page == undefined) {
					modalobj.modal('hide');
	...
	util.storelinkBrowser = function(callback, page, key){
		var footer = '';
		var modalobj = util.dialog('请选择商户',['./index.php?c=utility&a=newlink&do=storelink&callback=storeLinkComplete&page='+ page+'&keyword='+key],footer,{containerName:'link-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-body').css({'height':'700px','overflow-y':'auto' });
		modalobj.modal('show');

		window.storeLinkComplete = function(link, page ,key){
			if($.isFunction(callback)){
				callback(link, page ,key);
				if (page == '' || page == undefined) {
					modalobj.modal('hide');
	...
	util.foodBrowser = function(callback, page){
		var footer = '';
		var modalobj = util.dialog('请选择商户',['./index.php?c=utility&a=newlink&do=foodlink&callback=foodinkComplete&page='+ page],footer,{containerName:'link-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-body').css({'height':'700px','overflow-y':'auto' });
		modalobj.modal('show');
	...
	util.foodlinkBrowser = function(callback, page, key){
		var footer = '';
		var modalobj = util.dialog('请选择商户',['./index.php?c=utility&a=newlink&do=foodlink&callback=foodLinkComplete&page='+ page+'&keyword='+key],footer,{containerName:'link-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-body').css({'height':'700px','overflow-y':'auto' });
		modalobj.modal('show');

		window.foodLinkComplete = function(link, page ,key){
			if($.isFunction(callback)){
				callback(link, page ,key);
				if (page == '' || page == undefined) {
					modalobj.modal('hide');
	...
	util.altiveBrowser = function(callback, page){
		var footer = '';
		var modalobj = util.dialog('请选择活动',['./index.php?c=utility&a=newlink&do=altivelink&callback=altiveLinkComplete&page='+ page],footer,{containerName:'link-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-body').css({'height':'700px','overflow-y':'auto' });
		modalobj.modal('show');

		window.altiveLinkComplete = function(link, page){
			if($.isFunction(callback)){
				callback(link, page);
				if (page == '' || page == undefined) {
					modalobj.modal('hide');
	...
	util.altivelinkBrowser = function(callback, page, key){
		var footer = '';
		var modalobj = util.dialog('请选择活动',['./index.php?c=utility&a=newlink&do=altivelink&callback=altiveLinkComplete&page='+ page+'&keyword='+key],footer,{containerName:'link-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-body').css({'height':'700px','overflow-y':'auto' });
		modalobj.modal('show');

		window.altiveLinkComplete = function(link, page ,key){
			if($.isFunction(callback)){
				callback(link, page ,key);
				if (page == '' || page == undefined) {
					modalobj.modal('hide');
