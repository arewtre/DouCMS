...
function log_file($filepath,$msg){
    ...
    $file = fopen($dirpath.'/'.$filename, 'a+');
    $str = "[".date("Y/m/d h:i:s",time())."]".$msg."\n";
    fwrite($file, $str);
    fclose($file);
}
function insert_log($table,$data,$uid,$username) {
//     $sql = 'insert into '.'ims_'.$table. ' set ';
//     $condtion = '';
//     $i = 0;
//     foreach ($data as $key => $value){
//         $i++;
//         $condtion = $condtion.$key."=".$value;
//         if($i != count($data)){
//             $condtion = $condtion.",";
//         }
//     }
    $log_str ="用户ID:".$uid.","."用户名：".$username.","."添加了数据".","."数据表:"."ims_".$table.","."添加数据：".serialize($data);
    return $log_str;
}

function update_log($table,$data,$uid,$username,$old_data) {
//     $sql = 'update '.'ims_'.$table.' set ';
//     $condtion = '';
//     $i = 0;
//     foreach ($data as $key => $value){
//         $i++;
...
//         }
//     }
//     $sql = $sql.$condtion." where id = ".$id;
    $log_str ="用户ID:".$uid.","."用户名：".$username.","."更新了数据".","."数据表:"."ims_".$table.","."旧数据：".serialize($old_data).","."更新数据：".serialize($data);
    return $log_str;
}

function delete_log($table,$uid,$username,$old_data) {
    $log_str = "用户ID:".$uid.","."用户名：".$username.","."删除了数据".","."数据表:"."ims_".$table.","."删除数据：".serialize($old_data);
    return $log_str;
}
