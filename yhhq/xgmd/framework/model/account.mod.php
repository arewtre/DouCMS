...
{
    $groupid = pdo_fetchcolumn('SELECT groupid FROM ' . tablename('users') . ' WHERE uid = :uid', array(
        ':uid' => $uid
    ));
    $groupdata = pdo_fetch('SELECT maxaccount, maxsubaccount FROM ' . tablename('users_group') . ' WHERE id = :id', array(
        ':id' => $groupid
    ));
    $list = pdo_fetchall('SELECT uniacid FROM ' . tablename('uni_account_users') . ' WHERE uid = :uid AND role = :role ', array(
        ':uid' => $uid,
        ':role' => 'owner'
    ));
...
function uni_owned($uid = 0)
{
    ...
    if (in_array($uid, $founders)) {
        ...
        $uniacids = pdo_fetchall("SELECT uniacid FROM " . tablename('uni_account_users') . " WHERE uid = :uid", array(
            ':uid' => $uid
        ), 'uniacid');
        if (! empty($uniacids)) {
...
function uni_accounts($uniacid = 0)
{
    global $_W;
    $uniacid = empty($uniacid) ? $_W['uniacid'] : intval($uniacid);
    $accounts = pdo_fetchall("SELECT w.*, a.type, a.isconnect FROM " . tablename('account') . " a INNER JOIN " . tablename('account_wechats') . " w USING(acid) WHERE a.uniacid = :uniacid AND a.isdeleted <> 1 ORDER BY a.acid ASC", array(
        ':uniacid' => $uniacid
    ), 'acid');
    return $accounts;
}
...
function uni_fetch($uniacid = 0)
{
    ...
    $owneruid = pdo_fetchcolumn("SELECT uid FROM " . tablename('uni_account_users') . " WHERE uniacid = :uniacid AND role = 'owner'", array(
        ':uniacid' => $uniacid
    ));
    load()->model('user');
    $owner = user_single(array(
        'uid' => $owneruid
    ));
    ...
    $account['grouplevel'] = pdo_fetchcolumn('SELECT grouplevel FROM ' . tablename('uni_settings') . ' WHERE uniacid = :uniacid', array(
        ':uniacid' => $uniacid
    ));
...
function uni_modules($enabledOnly = true)
{
    ...
    $extend = pdo_getall('uni_account_group', array(
        'uniacid' => $_W['uniacid']
    ), array(), 'groupid');
    if (! empty($extend)) {
        $groupid = '-2';
    ...
    if (empty($groupid)) {
        $modules = pdo_fetchall("SELECT * FROM " . tablename('modules') . " WHERE issystem = 1 and is_display > 0 ORDER BY issystem DESC, mid ASC", array(), 'name');
    } elseif ($groupid == '-1') {
        $modules = pdo_fetchall("SELECT * FROM " . tablename('modules') . " where is_display > 0 ORDER BY issystem DESC, mid ASC", array(), 'name');
    } else {
        $group = pdo_fetch("SELECT id, name, package FROM " . tablename('users_group') . " WHERE id = :id", array(
            ':id' => $groupid
        ));
        if (! empty($group)) {
        ...
        if (in_array('-1', $packageids)) {
            ...
            $wechatgroup = pdo_fetchall("SELECT `modules` FROM " . tablename('uni_group') . " WHERE and id IN ('" . implode("','", $packageids) . "') OR uniacid = '{$_W['uniacid']}'");
            $ms = array();
            $mssql = '';
            ...
            $modules = pdo_fetchall("SELECT * FROM " . tablename('modules') . " WHERE is_display > 0 and issystem = 1{$mssql} ORDER BY issystem DESC, mid ASC", array(), 'name');
        }
    }
    if (! empty($modules)) {
...
function uni_modules_app_binding()
{
    ...
    if (! empty($modules)) {
        foreach ($modules as $module) {
            ...
            $entries = module_app_entries($module['name'], array(
                'home',
                'profile',
                'shortcut',
                'function',
...
function uni_setting_save($name, $value)
{
    ...
    $unisetting = pdo_get('uni_settings', array(
        'uniacid' => $_W['uniacid']
    ), array(
        'uniacid'
    ));
...
function uni_account_default($uniacid = 0)
{
    global $_W;
    $uniacid = empty($uniacid) ? $_W['uniacid'] : intval($uniacid);
    $account = pdo_fetch("SELECT w.*, a.default_acid FROM " . tablename('uni_account') . " a LEFT JOIN " . tablename('account_wechats') . " w ON a.default_acid = w.acid WHERE a.uniacid = :uniacid", array(
        ':uniacid' => $uniacid
    ), 'acid');
    if (empty($account['acid'])) {
        $default_acid = pdo_fetchcolumn("SELECT acid FROM " . tablename('account_wechats') . " WHERE uniacid = :uniacid ORDER BY level DESC", array(
            ':uniacid' => $_W['uniacid']
        ));
        $account = pdo_fetch("SELECT w.* FROM " . tablename('uni_account') . " AS a, " . tablename('account_wechats') . " AS w WHERE w.acid = '{$default_acid}'");
    }
    $account['type'] = pdo_fetchcolumn("SELECT type FROM " . tablename('account') . " WHERE acid = :acid", array(
        ':acid' => $account['acid']
    ));
    return $account;
}
...
function uni_user_permission_exist($uid = 0, $uniacid = 0)
{
    ...
    $is_exist = pdo_fetch('SELECT id FROM ' . tablename('users_permission') . ' WHERE `uid`=:uid AND `uniacid`=:uniacid', array(
        ':uid' => $uid,
        ':uniacid' => $uniacid
    ));
    if (empty($is_exist)) {
...
function uni_user_permission_check($permission_name, $is_html = true, $action = '')
{
    ...
    if ($action == 'reply') {
        ...
        $entry = pdo_fetch('SELECT * FROM ' . tablename('modules_bindings') . ' WHERE `eid`=:eid', array(
            ':eid' => $eid
        ));
        if (! empty($entry)) {
...
function uni_user_module_permission_check($action = '', $module_name = '')
{
    ...
    $m = $_GPC['m'];
    if (! empty($do) && ! empty($m)) {
        $is_exist = pdo_fetch('SELECT eid FROM ' . tablename('modules_bindings') . ' WHERE module=:module AND do = :do AND entry = :entry', array(
            ':module' => $m,
            ':do' => $do,
            ':entry' => 'menu'
        ));
...
function uni_update_week_stat()
{
    ...
    $week_stat_fans = pdo_getall('stat_fans', array(
        'date' => $seven_days,
        'uniacid' => $_W['uniacid']
    ), '', 'date');
    ...
    foreach ($seven_days as $sevens) {
        if ($_W['account']['level'] == ACCOUNT_SUBSCRIPTION_VERIFY || $_W['account']['level'] == ACCOUNT_SERVICE_VERIFY) {
            ...
            $update_stat = array();
            $update_stat['cumulate'] = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('mc_mapping_fans') . " WHERE acid = :acid AND uniacid = :uniacid AND follow = :follow AND followtime < :endtime", array(
                ':acid' => $_W['acid'],
                ':uniacid' => $_W['uniacid'],
                ':endtime' => strtotime($sevens) + 86400,
                ':follow' => 1
            ));
        ...
        if (empty($week_stat_fans[$sevens])) {
            ...
            pdo_update('stat_fans', $update_stat, array(
                'id' => $week_stat_fans[$sevens]['id']
            ));
        }
    }
...
{
    $account = pdo_fetch("SELECT w.*, a.type, a.isconnect FROM " . tablename('account') . " a INNER JOIN " . tablename('account_wechats') . " w USING(acid) WHERE acid = :acid AND a.isdeleted = '0'", array(
        ':acid' => $acid
    ));
    if (empty($account)) {
...
{
    global $_W;
    $response = account_weixin_http($username, WEIXIN_ROOT . '/advanced/callbackprofile?t=ajax-response&lang=zh_CN', array(
    ...
    $response = account_weixin_http($username, WEIXIN_ROOT . '/misc/skeyform?form=advancedswitchform', array(
        'f' => 'json',
        'lang' => 'zh_CN',
        'flag' => '1',
        'type' => '2',
...
function account_weixin_http($username, $url, $post = '')
{
    ...
    $auth = $_W['cache'];
    return ihttp_request($url . '&token=' . $auth['wxauth:' . $username . ':token'], $post, array(
        'CURLOPT_COOKIE' => $auth['wxauth:' . $username . ':cookie'],
        'CURLOPT_REFERER' => WEIXIN_ROOT . '/advanced/advanced?action=edit&t=advanced/edit&token=' . $auth['wxauth:' . $username . ':token']
    ));
}
...
function uni_setmeal($uniacid = 0)
{
    ...
    load()->model('user');
    $groups = pdo_getall('users_group', array(), array(
        'id',
        'name'
    ), 'id');
    $owner = user_single(array(
...
function uni_is_multi_acid($uniacid = 0)
{
    ...
    $nums = intval($nums);
    if (! $nums) {
        $nums = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('account_wechats') . ' WHERE uniacid = :uniacid', array(
            ':uniacid' => $_W['uniacid']
        ));
...
function account_delete($acid)
{
    global $_W;
    load()->func('file');
    $account = pdo_get('uni_account', array(
        'default_acid' => $acid
    ));
    if ($account) {
        $uniacid = $account['uniacid'];
        ...
        $subaccount = pdo_fetchall("SELECT acid FROM " . tablename('account') . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $uniacid
        ));
        if (! empty($subaccount)) {
        ...
        $tables = array(
            ...
            'account_wechats',
            'activity_coupon',
            'activity_coupon_allocation',
            'activity_coupon_modules',
            'activity_clerks',
            'activity_coupon_record',
            'activity_exchange',
        ...
        pdo_delete('account', array(
            'acid' => $acid
        ));
        pdo_delete('account_wechats', array(
            'acid' => $acid,
            'uniacid' => $uniacid
        ));
        cache_delete("unicount:{$uniacid}");
        ...
        $oauth = uni_setting($uniacid, array(
            'oauth'
        ));
        if ($oauth['oauth']['account'] == $acid) {
            $acid = pdo_fetchcolumn('SELECT acid FROM ' . tablename('account_wechats') . " WHERE uniacid = :id AND level = 4 AND secret != '' AND `key` != ''", array(
                ':id' => $uniacid
            ));
            pdo_update('uni_settings', array(
                'oauth' => iserializer(array(
                    'account' => $acid,
...
{
    $group = pdo_get('users_group', array(
        'id' => $groupid
    ));
    if (empty($group)) {
        return '';
        ...
        if (in_array(- 1, $package)) {
            ...
            foreach ($package as $value) {
                $values = pdo_get('uni_group', array(
                    'id' => $value
                ), 'modules');
