...
function module_entries($name, $types = array(), $rid = 0, $args = null) {
	...
	$sql = 'SELECT * FROM ' . tablename('modules_bindings')." WHERE `module`=:module and is_display < 1 AND `entry` IN ('{$fields}') ORDER BY displayorder DESC, eid ASC";
	$pars = array();
	$pars[':module'] = $name;
...
function module_app_entries($name, $types = array(), $args = null) {
	...
	$fields = implode("','", $types);
	$sql = 'SELECT * FROM ' . tablename('modules_bindings')." WHERE `module`=:module and is_display < 1 AND `entry` IN ('{$fields}') ORDER BY eid ASC";
	$pars = array();
	$pars[':module'] = $name;
	$bindings = pdo_fetchall($sql, $pars);
	$entries = array();
