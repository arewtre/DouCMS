...
class DB {
	...
	public function getall($tablename, $params = array(), $fields = array(), $keyfield = '') {
		...
		$condition = $this->implode($params, 'AND');
		$sql = "SELECT {$select} FROM " .$this->tablename($tablename) . (!empty($condition['fields']) ? " WHERE {$condition['fields']}" : '');
	...
	public function update($table, $data = array(), $params = array(), $glue = 'AND') {
		...
		$item = pdo_fetchall("select * from ".$this->tablename($table)." where ".$condition['fields'],$condition['params']);
		...
		if($result == 1){
    		...
    		if($_GPC['m'] != null){
    		  $filepath = IA_ROOT.'/data/logs/'.$_GPC['m'].'/'.$_GPC['do'].'/'.date('Ymd',time()).'.txt';
    		}else{
		      $filepath = IA_ROOT.'/data/logs/'.$_GPC['c'].'/'.$_GPC['a'].'/'.date('Ymd',time()).'.txt';
    		}
    		$log_str = update_log($table, $data, $uid,$username,$item);
    		log_file($filepath, $log_str);
		}
		return $result;
	}
	
	public function update2($table, $data = array(), $params = array(), $glue = 'AND') {
	    $fields = $this->implode($data, ',');
	    $condition = $this->implode($params, $glue);
	    $params = array_merge($fields['params'], $condition['params']);
	    $sql = "UPDATE " . $this->tablename2($table) . " SET {$fields['fields']}";
	    $sql .= $condition['fields'] ? ' WHERE '.$condition['fields'] : '';
	
	
	    $item = pdo_fetchall("select * from ".$this->tablename2($table)." where ".$condition['fields'],$condition['params']);
	    $result = $this->query($sql, $params);
	    if($result == 1){
	        global $_GPC,$_W;
	        $uid = $_W['uid'];
	...
	public function insert($table, $data = array(), $replace = FALSE) {
		...
		$result = $this -> query("$cmd " . $this->tablename($table) . " SET {$condition['fields']}", $condition['params']);
		if($result == 1){
    		global $_GPC,$_W;
    		$uid = $_W['uid'];
    		$username = $_W['username'];
    		$filepath = IA_ROOT.'/data/logs/'.$_GPC['m'].'/'.$_GPC['do'].'/'.date('Ymd',time()).'.txt';
    		$log_str = insert_log($table, $data, $uid,$username);
    		log_file($filepath, $log_str);
		}
		return $result;
	}
	
	public function insert2($table, $data = array(), $replace = FALSE) {
	    $cmd = $replace ? 'REPLACE INTO' : 'INSERT INTO';
	    $condition = $this->implode($data, ',');
	    $result = $this -> query("$cmd " . $this->tablename2($table) . " SET {$condition['fields']}", $condition['params']);
	    if($result == 1){
	        global $_GPC,$_W;
	        $uid = $_W['uid'];
	        $username = $_W['username'];
	...
	public function delete($table, $params = array(), $glue = 'AND') {
		...
		$item = pdo_fetchall('select * from '.$this->tablename($table).' where '.$condition['fields'],$condition['params']);
		$result = $this->query($sql, $condition['params']);
		if($result == 1){
		    global $_GPC,$_W;
    		$uid = $_W['uid'];
    		$username = $_W['username'];
		    $filepath = IA_ROOT.'/data/logs/'.$_GPC['m'].'/'.$_GPC['do'].'/'.date('Ymd',time()).'.txt';
		    $log_str = delete_log($table, $uid,$username, $item);
		    log_file($filepath, $log_str);
		}
		return $this->query($sql, $condition['params']);
	}
	public function delete2($table, $params = array(), $glue = 'AND') {
	    $condition = $this->implode($params, $glue);
	    $sql = "DELETE FROM " . $this->tablename2($table);
	    $sql .= $condition['fields'] ? ' WHERE '.$condition['fields'] : '';
	...
	public function implode($params, $glue = ',') {
	...
	public function tablename2($table) {
	    return "`{$table}`";
	}
