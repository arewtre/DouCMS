(function(window) {
	...
	util.iconBrowser = function(callback){
		...
		var modalobj = util.dialog('请选择图标',['./index.php??m=Home&c=Utility&a=icon&callback=selectIconComplete'],footer,{containerName:'icon-container'});
		modalobj.modal({'keyboard': false});
	...
	util.emojiBrowser = function(callback){
		var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>';
		var modalobj = util.dialog('请选择表情',['./index.php?m=Home&c=Utility&a=emoji&callback=selectEmojiComplete'],footer,{containerName:'icon-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-dialog').css({'width':'70%'});
		modalobj.find('.modal-body').css({'height':'70%','overflow-y':'scroll'});
		modalobj.modal('show');
	...
	util.couponModuleLink = function(callback,couponid){
		var footer = '';
		var modalobj = util.dialog('查看二维码',['./index.php?c=utility&a=link&do=couponlink&callback=moduleLinkComplete&couponid='+ couponid], '');
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-body').css({'height':'620px','overflow-y':'auto' });
		modalobj.modal('show');
		window.moduleLinkComplete = function(link, couponid){
			if($.isFunction(callback,couponid)){
				callback(link, couponid);
				modalobj.modal('hide');
			}
		};
	};
	
	util.goodslinkBrowser = function(callback, page){
		var footer = '';
		var modalobj = util.dialog('',['./index.php?c=utility&a=link&do=goods&callback=goodsLinkComplete&page='+ page],footer,{containerName:'link-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-body').css({'height':'700px','overflow-y':'auto' });
		modalobj.modal('show');

		window.goodsLinkComplete = function(link, page){
			if($.isFunction(callback)){
				callback(link, page);
				if (page == '' || page == undefined) {
					modalobj.modal('hide');
	...
	util.goodBrowser = function(callback, page,key){
		var footer = '';
		var modalobj = util.dialog('',['./index.php?c=utility&a=link&do=goods&callback=goodsLinkComplete&page='+ page+'&keyword='+ key],footer,{containerName:'link-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-body').css({'height':'700px','overflow-y':'auto' });
		modalobj.modal('show');

		window.goodsLinkComplete = function(link, page,key){
			if($.isFunction(callback)){
	...
	util.couponlinkBrowser = function(callback, page){
		var footer = '';
		var modalobj = util.dialog('',['./index.php?c=utility&a=link&do=coupon&callback=couponLinkComplete&page='+ page],footer,{containerName:'link-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-body').css({'height':'700px','overflow-y':'auto' });
		modalobj.modal('show');

		window.couponLinkComplete = function(link, page){
			if($.isFunction(callback)){
				callback(link, page);
				if (page == '' || page == undefined) {
					modalobj.modal('hide');
