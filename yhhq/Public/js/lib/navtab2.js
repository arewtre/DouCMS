			...
			if(that.config.closed) {
				//监听关闭事件
				    ELEM.titleBox.find('li').children('i.layui-tab-close[data-id=' + globalTabIdIndex + ']').on('click', function() {
				    	elements.tabDelete(ELEM.tabFilter, $(this).parent('li').index()).init();
				    });
			    };
	 ...
	 LarryTab.prototype.tabAdd = function(data){
         ...
         var _config = that.config;
		    ...
		    if(tabIndex === -1){
			    ...
			    //切换到当前打开的选项卡
			    elements.tabChange(ELEM.tabFilter, ELEM.titleBox.find('li').length - 1);
		    }else {
			    elements.tabChange(ELEM.tabFilter, tabIndex);
		    }
     };
